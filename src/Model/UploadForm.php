...
/**
 * Модель загрузки файла пакета расширения модуля.
 * 
...
class UploadForm extends FormModel
{
        ...
        if ($isValid) {
            ...
            if ($file->validate()) {
                ...
                if (!$this->makePackageFolder($file->getBaseName())) {
                    $this->setError(
                        $this->module->t('Error creating temporary directory to download module extension package file')
                    );
                    return false;    
                }
            } else {
    ...
    /**
     * Создаёт временный каталог расширения модуля.
     * 
     * @param string $packageFolder Имя папки расширения модуля.
     * 
     * @return bool
     */
    ...
    /**
     * Извлечение файлов из пакета расширения модуля.
     * 
     * @return bool
     */
    protected function unpackProcess(): bool
    {
        ...
        if (empty($package->id) || empty($package->type)) {
            $this->setError(
                $this->module->t('The module extension package file does not contain one of the attributes: id, type')
            );
            return false;
        }
        ...
        if ($package->type !== 'extension') {
            $this->setError(
                $this->module->t('Module extension attribute "{0}" is incorrectly specified', ['type'])
            );
            return false;
        }

        /** @var array|null $params Параметры установленного расширения модуля */
        $params = Gm::$app->extensions->getRegistry()->get($package->id);
        if ($params) {
            /** @var array|null $names */
            $names = Gm::$app->extensions->selectName($params['rowId']);
            $this->setError(
                $this->module->t(
