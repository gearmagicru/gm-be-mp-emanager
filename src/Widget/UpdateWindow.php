...
/**
 * Виджет для формирования интерфейса обновления расширения модуля.
 * 
...
class UpdateWindow extends Window
{
    ...
    /**
     * Маршрут заврешения обновления расширения.
     * 
     * @var string
     */
    public string $route = '';
    ...
    protected function init(): void
    {
        ...
        $this->form = new Form([
            'id'     => 'update-form', // => gm-mp-emanager-update-form
            'router' => [
                'route' => $this->route,
        ...
        $this->id        = 'update'; // => gm-mp-emanager-update
        $this->ui        = 'light';
        $this->cls       = 'g-emanager-update';
    ...
    /**
     * Возвращает шаги обновления расширения.
     * 
     * @return array
     */
    public function renderSteps(): array
    {
        ...
        foreach ($this->steps as $step) {
            ...
            $html .= '<div class="gm-mp-emanager-update__step' . $active .'"><i class="fal fa-check"></i> ' . $title . '</div>';
        }
        return [
            'xtype' => 'container',
            'cls'   => 'gm-mp-emanager-update__steps',
            'html'  => $html
        ];
    }
    ...
    public function renderFormItems(): array
    {
        return [
            ...
            [
                'xtype' => 'container',
                'cls'   => 'gm-mp-emanager-update__notice',
                'html'  => '#{update.notice}'
            ],
            [
                'xtype' => 'hidden',
